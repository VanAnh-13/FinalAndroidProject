    ...
    protected <T> void executeTask(
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            if (response instanceof DataState.Success) {
                ...
                DataState.Success<T> success = (DataState.Success<T>) response;
                onSuccess.accept(success.getData());
                hideLoading();
            } else if (response instanceof DataState.Error) {
                // Extract exception from error response and call error callback
                DataState.Error error = (DataState.Error) response;
                onError.accept(error.getException());
                hideLoading();
            }
        });
    }
