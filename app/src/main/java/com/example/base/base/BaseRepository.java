    ...
    protected <T> CompletableFuture<DataState<T>> getResult(Supplier<T> request) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                // Wrap successful result in DataState.Success
                return (DataState<T>) new DataState.Success<>(result);
            } catch (Exception exception) {
                // Wrap any exception in DataState.Error
                return (DataState<T>) new DataState.Error(exception);
            }
        }, executorService);
    }
}