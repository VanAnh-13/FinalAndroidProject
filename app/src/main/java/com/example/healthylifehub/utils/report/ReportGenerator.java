...
public class ReportGenerator {
    ...
    private ReportData fetchReportData(String userId, Date startDate, Date endDate) {
        ...
        try {
            ...
            // Fetch medical records within date range
            Task<QuerySnapshot> recordsTask = firestore
                    .collection("users").document(userId)
            ...
            Tasks.await(recordsTask);
            
            List<com.example.healthylifehub.data.model.MedicalRecord> medicalRecords = 
                    recordsTask.getResult().toObjects(com.example.healthylifehub.data.model.MedicalRecord.class);
            reportData.setMedicalRecords(medicalRecords);
            
            // Fetch active medicines (current medications at report time)
            Task<QuerySnapshot> medicinesTask = firestore
                    .collection("users").document(userId)
                    .collection("medicines")
                    .whereEqualTo("status", "ACTIVE")
                    .get();
            
            Tasks.await(medicinesTask);
            
            List<com.example.healthylifehub.data.model.Medicine> medicines = 
                    medicinesTask.getResult().toObjects(com.example.healthylifehub.data.model.Medicine.class);
            reportData.setMedicines(medicines);
            
            // Fetch user profile
            Task<com.google.firebase.firestore.DocumentSnapshot> profileTask = firestore
                    .collection("users")
                    .document(userId)
                    .get();
            
            Tasks.await(profileTask);
            
            if (profileTask.getResult() != null && profileTask.getResult().exists()) {
                // Extract profile data from user document
                com.google.firebase.firestore.DocumentSnapshot doc = profileTask.getResult();
                com.example.healthylifehub.data.model.UserProfile userProfile = 
                        new com.example.healthylifehub.data.model.UserProfile();
                
                // Get profile nested object or use direct fields
                if (doc.contains("profile")) {
                    @SuppressWarnings("unchecked")
                    java.util.Map<String, Object> profileMap = 
                            (java.util.Map<String, Object>) doc.get("profile");
                    
                    if (profileMap != null) {
                        userProfile.setFullName((String) profileMap.get("fullName"));
                        userProfile.setDateOfBirth((String) profileMap.get("dateOfBirth"));
                        userProfile.setGender((String) profileMap.get("gender"));
                        userProfile.setHeight(profileMap.get("height") != null ? 
                                ((Number) profileMap.get("height")).doubleValue() : 0);
                        userProfile.setWeight(profileMap.get("weight") != null ? 
                                ((Number) profileMap.get("weight")).doubleValue() : 0);
                        userProfile.setBloodType((String) profileMap.get("bloodType"));
                        userProfile.setMedicalHistory((String) profileMap.get("medicalHistory"));
                    }
                }
                ...
                if (userProfile.getHeight() > 0 && userProfile.getWeight() > 0) {
                    ...
                    userProfile.setBmi(userProfile.getWeight() / (heightInMeters * heightInMeters));
                }
                
                reportData.setUserProfile(userProfile);
            }
    ...
    private Bitmap generateHeartRateChart(List<HealthMetric> metrics) {
        ...
        LineDataSet dataSet = new LineDataSet(entries, context.getString(R.string.field_heart_rate));
        ...
        LineData lineData = new LineData(dataSet);
        chart.setData(lineData);
        chart.getDescription().setText(context.getString(R.string.chart_heart_rate));
        chart.invalidate();
    ...
    private Bitmap generateWeightChart(List<HealthMetric> metrics) {
        ...
        LineDataSet dataSet = new LineDataSet(entries, context.getString(R.string.field_weight));
        ...
        LineData lineData = new LineData(dataSet);
        chart.setData(lineData);
        chart.getDescription().setText(context.getString(R.string.chart_weight));
        chart.invalidate();
    ...
    private Bitmap generateTemperatureChart(List<HealthMetric> metrics) {
        ...
        LineDataSet dataSet = new LineDataSet(entries, context.getString(R.string.field_temperature));
        ...
        LineData lineData = new LineData(dataSet);
        chart.setData(lineData);
        chart.getDescription().setText(context.getString(R.string.chart_temperature));
        chart.invalidate();
    ...
    private File createPDFWithiText(ReportData reportData) throws Exception {
        ...
        try {
            ...
            if (reportData.getUserProfile() != null) {
                com.example.healthylifehub.data.model.UserProfile profile = reportData.getUserProfile();
                
                document.add(new com.itextpdf.layout.element.Paragraph("Thông tin cá nhân")
                    .setFontSize(16)
                    .setBold());
                document.add(new com.itextpdf.layout.element.Paragraph("\n"));
                
                com.itextpdf.layout.element.Table profileTable = 
                    new com.itextpdf.layout.element.Table(2);
                profileTable.setWidth(com.itextpdf.layout.properties.UnitValue.createPercentValue(100));
                
                if (profile.getFullName() != null) {
                    profileTable.addCell("Họ và tên");
                    profileTable.addCell(profile.getFullName());
                }
                if (profile.getEmail() != null) {
                    profileTable.addCell("Email");
                    profileTable.addCell(profile.getEmail());
                }
                if (profile.getDateOfBirth() != null) {
                    profileTable.addCell("Ngày sinh");
                    profileTable.addCell(profile.getDateOfBirth());
                }
                if (profile.getGender() != null && !profile.getGender().isEmpty()) {
                    profileTable.addCell("Giới tính");
                    profileTable.addCell(profile.getGender());
                }
                if (profile.getHeight() > 0) {
                    profileTable.addCell("Chiều cao");
                    profileTable.addCell(String.format("%.1f cm", profile.getHeight()));
                }
                if (profile.getWeight() > 0) {
                    profileTable.addCell("Cân nặng");
                    profileTable.addCell(String.format("%.1f kg", profile.getWeight()));
                }
                if (profile.getBmi() > 0) {
                    profileTable.addCell("BMI");
                    profileTable.addCell(String.format("%.2f", profile.getBmi()));
                }
                if (profile.getBloodType() != null && !profile.getBloodType().isEmpty()) {
                    profileTable.addCell("Nhóm máu");
                    profileTable.addCell(profile.getBloodType());
                }
            ...
            if (reportData.getMedicines() != null && !reportData.getMedicines().isEmpty()) {
                document.add(new com.itextpdf.layout.element.Paragraph("Thuốc đang sử dụng")
                    .setFontSize(16)
                    .setBold());
                document.add(new com.itextpdf.layout.element.Paragraph("\n"));
                
                com.itextpdf.layout.element.Table medicinesTable = 
                    new com.itextpdf.layout.element.Table(4);
                medicinesTable.setWidth(com.itextpdf.layout.properties.UnitValue.createPercentValue(100));
                
                // Header row
                medicinesTable.addHeaderCell(
                    new com.itextpdf.layout.element.Cell()
                        .add(new com.itextpdf.layout.element.Paragraph("Tên thuốc"))
                        .setBold()
                        .setBackgroundColor(com.itextpdf.kernel.colors.ColorConstants.LIGHT_GRAY));
                medicinesTable.addHeaderCell(
                    new com.itextpdf.layout.element.Cell()
                        .add(new com.itextpdf.layout.element.Paragraph("Liều lượng"))
                        .setBold()
                        .setBackgroundColor(com.itextpdf.kernel.colors.ColorConstants.LIGHT_GRAY));
                medicinesTable.addHeaderCell(
                    new com.itextpdf.layout.element.Cell()
                        .add(new com.itextpdf.layout.element.Paragraph("Tần suất"))
                        .setBold()
                        .setBackgroundColor(com.itextpdf.kernel.colors.ColorConstants.LIGHT_GRAY));
                medicinesTable.addHeaderCell(
                    new com.itextpdf.layout.element.Cell()
                        .add(new com.itextpdf.layout.element.Paragraph("Hướng dẫn"))
                        .setBold()
                        .setBackgroundColor(com.itextpdf.kernel.colors.ColorConstants.LIGHT_GRAY));
                
                // Data rows
                for (com.example.healthylifehub.data.model.Medicine medicine : reportData.getMedicines()) {
                    medicinesTable.addCell(medicine.getName() != null ? medicine.getName() : "");
                    medicinesTable.addCell(medicine.getDosage() != null ? medicine.getDosage() : "");
                    medicinesTable.addCell(medicine.getFrequency() != null ? medicine.getFrequency() : "");
                    medicinesTable.addCell(medicine.getInstructions() != null ? medicine.getInstructions() : "");
                }
            ...
            if (reportData.getMedicalRecords() != null && !reportData.getMedicalRecords().isEmpty()) {
                document.add(new com.itextpdf.layout.element.Paragraph("Hồ sơ bệnh án")
                    .setFontSize(16)
                    .setBold());
                document.add(new com.itextpdf.layout.element.Paragraph("\n"));
                
                for (com.example.healthylifehub.data.model.MedicalRecord record : reportData.getMedicalRecords()) {
                    // Create record box/section
                    com.itextpdf.layout.element.Table recordTable = 
                        new com.itextpdf.layout.element.Table(2);
                    recordTable.setWidth(com.itextpdf.layout.properties.UnitValue.createPercentValue(100));
                    recordTable.setBorder(new com.itextpdf.layout.borders.SolidBorder(1));
                    
                    if (record.getTitle() != null) {
                        recordTable.addCell(
                            new com.itextpdf.layout.element.Cell(1, 2)
                                .add(new com.itextpdf.layout.element.Paragraph(record.getTitle())
                                    .setBold()
                                    .setFontSize(14))
                                .setBackgroundColor(com.itextpdf.kernel.colors.ColorConstants.LIGHT_GRAY));
                    }
                    ...
                    if (record.getDate() != null) {
                        recordTable.addCell("Ngày khám");
                        recordTable.addCell(record.getDate());
                    }
                    if (record.getHospital() != null) {
                        recordTable.addCell("Bệnh viện");
                        recordTable.addCell(record.getHospital());
                    }
                    if (record.getDoctor() != null) {
                        recordTable.addCell("Bác sĩ");
                        recordTable.addCell(record.getDoctor());
                    }
                    if (record.getDiagnosis() != null) {
                        recordTable.addCell("Chẩn đoán");
                        recordTable.addCell(record.getDiagnosis());
                    }
                    if (record.getDescription() != null) {
                        recordTable.addCell("Mô tả");
                        recordTable.addCell(record.getDescription());
                    }
                    if (record.getType() != null) {
                        recordTable.addCell("Loại");
                        recordTable.addCell(record.getType().toString());
                    }
